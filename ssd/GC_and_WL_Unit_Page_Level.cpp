...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Page_Level::Check_gc_required(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		...
		if (free_block_pool_size < block_pool_gc_threshold)
		{
			...
			// edit
		 	Flash_Transaction_Queue* GCTRQueue = tsu->Get_GCEraseTRQueue( plane_address.ChannelID, plane_address.ChipID ) ;
			// edit
			...
			case SSD_Components::GC_Block_Selection_Policy_Type::RGA:
			{
				...
				// while (random_set.size() < block_no_per_plane)
				{
			...
			float percent_of_invalid = (float)pbke->Invalid_pages_count / (float)pbke->Total_pages_count;
			// second edit
			float percent_of_valid = (float)pbke->Valid_pages_count / (float)pbke->Total_pages_count;
			float percent_of_free = (float)pbke->Free_pages_count / (float)pbke->Total_pages_count;
			...
			if (block->Current_page_write_index == 0 || block->Invalid_page_count == 0){//No invalid page to erase
				...
				fs << Simulator->Time() << "\t" << plane_address.ChannelID << "\t" << plane_address.ChipID << "\t" << plane_address.DieID << "\t" << plane_address.PlaneID << "\tNon-GC\t" << free_block_pool_size << "\t" << percent_of_invalid << "\t" << percent_of_valid << "\t" << percent_of_free << "\t" << block->Invalid_page_count << "\t" << GCTRQueue->GetNumberOfRequests() << std::endl ;
				*/
				return;
			} // if

			// add cooment if record data ****	
			...
			// Free block space < Hard Threshold						
				   ...
				   percent_of_invalid, percent_of_valid, percent_of_free, block->Invalid_page_count, GCTRQueue->GetNumberOfRequests())) {
			} // else if
			...
			if (block_manager->Can_execute_gc_wl(gc_candidate_address))//If there are ongoing requests targeting the candidate block, the gc execution should be postponed
			{
				/*			
				float percent_of_invalid = (float)pbke->Invalid_pages_count / (float)pbke->Total_pages_count;
				// fs << Simulator->Time() << " GC!! free_block_size: " << free_block_pool_size <<" percent: " << percent_of_invalid << std::endl ;
				// second edit
				float percent_of_valid = (float)pbke->Valid_pages_count / (float)pbke->Total_pages_count;
				float percent_of_free = (float)pbke->Free_pages_count / (float)pbke->Total_pages_count;

				
				fs << Simulator->Time() << "\t" << plane_address.ChannelID << "\t" << plane_address.ChipID << "\t" << plane_address.DieID << "\t" << plane_address.PlaneID << "\tGC\t" << free_block_pool_size << "\t" << percent_of_invalid << "\t" << percent_of_valid << "\t" << percent_of_free << "\t" << block->Invalid_page_count << "\t" << GCTRQueue->GetNumberOfRequests() << std::endl ;
				*/	
		...
		{ // Non-gc
				...
				float percent_of_valid = (float)pbke->Valid_pages_count / (float)pbke->Total_pages_count;
				float percent_of_free = (float)pbke->Free_pages_count / (float)pbke->Total_pages_count;
				// fs << Simulator->Time() << " Non-GC!! free_block_size: " << free_block_pool_size <<" percent: " << percent_of_invalid << std::endl ;
				...
				fs << Simulator->Time() << "\t" << plane_address.ChannelID << "\t" << plane_address.ChipID << "\t" << plane_address.DieID << "\t" << plane_address.PlaneID << "\tNon-GC\t" << free_block_pool_size << "\t" << percent_of_invalid << "\t" << percent_of_valid << "\t" << percent_of_free << "\t" << "0" << "\t" << GCTRQueue->GetNumberOfRequests() << std::endl ;
			 	*/		
		}
	}
}
